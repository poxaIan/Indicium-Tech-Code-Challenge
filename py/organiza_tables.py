...
def extract_table_and_date(file_name):
    ...
    pattern = r'public-(?P<table_name>[a-zA-Z0-9_]+)-(?P<timestamp>\d{8}T\d{6})\.csv'
...
os.chmod('./data/postgres', 0o777)
source_dir = './data/postgres'
target_base_dir = './data/postgres'

for file_name in os.listdir(source_dir):
    if file_name.endswith('.csv'):
        table_name = extract_table_and_date(file_name)
        target_dir = os.path.join(target_base_dir,table_name,arg_date)
