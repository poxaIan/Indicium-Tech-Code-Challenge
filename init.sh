...
echo 'Extracting all .csv from disk to southwind'
...
# for the queries
docker exec -i challenge_db2_1 sh -c "
  psql -U southwind_user -d southwind -A -F ';' " < queries/fix_datatype.sql
...
for sql_file in queries/*.sql; do
    ...
    docker exec -i challenge_db2_1 sh -c "
     psql -U southwind_user -d southwind -A -F ';' " < $sql_file > "queries/$ARG_DATE/${base_name}.csv"
